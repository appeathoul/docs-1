...
The goal of this section is to familiarize you with the specifics of setting up ORY Hydra in your environment. Before starting with this section, please check out the [tutorial](https://github.com/appeathoul/docs-1/tree/03e1a8789cab7fa56b26bd8fdf45a1b91f76c21a/guides/1-hydra/0-Tutorial/0-README.md). It will teach you the most important flows and settings for Hydra.
...
4. Download and run an exemplary consent app \([node](https://github.com/ory/hydra-consent-app-express), [golang](https://github.com/appeathoul/docs-1/tree/03e1a8789cab7fa56b26bd8fdf45a1b91f76c21a/guides/1-hydra/3-install/hydra-consent-app-go/README.md)\).
...
This command wil start a postgres instance with name `ory-hydra-example--postgres`, set up a database called `hydra` and create a user `hydra` with password `secret`.

## Install and run ORY Hydra

We highly recommend using Docker to run Hydra, as installing, configuring and running Hydra is easiest with Docker. ORY Hydra is available on [Docker Hub](https://hub.docker.com/r/oryd/hydra/).
...
To confirm that the instance is running properly, [open the health check](https://localhost:9000/health/status). If asked, accept the self signed certificate in your browser. You should simply see `ok`.
...
You can also install ORY Hydra without docker. For the purpose of this tutorial, [please skip this section for now](0.md#configure-ory-hydra), and read it later.

#### Download binaries

The client and server **binaries are downloadable at the **[**releases tab**](https://github.com/ory/hydra/releases). There is currently no installer available. You have to add the Hydra binary to the PATH environment variable yourself or put the binary in a location that is already in your path \(`/usr/bin`, ...\). If you do not understand what that all of this means, ask in our [chat channel](https://gitter.im/ory-am/hydra). We are happy to help.
...
If you wish to compile ORY Hydra yourself, you need to install and set up [Go 1.8+](https://golang.org/) and add `$GOPATH/bin` to your `$PATH`. To do so, run the following commands in a shell \(bash, sh, cmd.exe, ...\):
...
4. we create another client capable of performing the client credentials grant and the authorize code grant.
...
The consent app is a bridge between ORY Hydra and your identity provider \(log in, sign up, reset password, ...\). The consent app requires a registered OAuth 2.0 Client at ORY Hydra. Do not re-use this client for any other purposes. To create the client, use the following command:
...
* `--secret consent-secret` sets the secret of the client. If no secret is provided, one will be generated.
  ...
  The secret is visible _only once and can not be retrieved again_.
...
Cool, next we need to create a policy for this client as well. ORY Hydra uses policies to decide whether a user is allowed to do something in the system or not. It is different from OAuth 2.0 Scopes, as those apply only to the access token itself, not the user. The following policy allows `consent-app` to access the required cryptographic keys for validating and signing the consent challenge and response:
...
Awesome! Next we will run the [ORY Hydra Consent App Example \(NodeJS\)](https://github.com/ory/hydra-consent-app-express). This app is also available in [Golang](https://github.com/ory/hydra-consent-app-go), but for the purpose of this tutorial we will use the NodeJS one:
...
Awesome, our infrastructure is set up! Now it's time to create an OAuth 2.0 Consumer and perform the OAuth 2.0 Authorize Code flow. To do so, we will create a new client:
...
open the link, as prompted, in your browser, and follow the steps shown there. When completed, you should land at a screen that looks like this one:

![OAuth 2.0 result](../../../.gitbook/assets/install-result.png)
